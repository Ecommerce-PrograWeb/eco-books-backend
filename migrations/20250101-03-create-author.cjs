...
module.exports = {
  async up(qi, Sequelize) {
    await qi.sequelize.transaction(async (t) => {
      ...
      await qi.addConstraint('Author', {
        fields: ['created_by'],
        type: 'foreign key',
        name: 'fk_author_created_by',
  ...
  async down(qi) {
    await qi.sequelize.transaction(async (t) => {
      await qi.removeConstraint('Author', 'fk_author_created_by', { transaction: t }).catch(() => {});
      await qi.dropTable('Author', { transaction: t });
    });
  }
};
